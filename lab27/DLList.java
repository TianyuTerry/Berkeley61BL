...
public class DLList<T extends Comparable<T>> {
    ...
    private void insertionSortHelper(T item) {
        ...
        while (ptr != sentinel && ptr.item.compareTo(item) > 0) {
            ptr = ptr.prev;
        }
        Node newNode = new Node(item, ptr, ptr.next);
    ...
    public DLList<T> mergeSort() {
        ...
        DLList<T> oneHalfSorted = oneHalf.mergeSort();
        DLList<T> otherHalfSorted = otherHalf.mergeSort();
        return oneHalfSorted.merge(otherHalfSorted);
    }
    ...
    public DLList<T> quicksort() {
        ...
        DLList<T> smallElementsSorted = smallElements.quicksort();
        DLList<T> largeElementsSorted = largeElements.quicksort();
        smallElementsSorted.append(equalElements);
        smallElementsSorted.addLast(pivot);
        smallElementsSorted.append(largeElementsSorted);
        return smallElementsSorted;
    }
